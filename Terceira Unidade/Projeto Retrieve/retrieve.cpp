...
/**
 * @brief Função recursiva para encontrar e escrever as sugestões.
 */
...
void buscar(NoTrie* raiz, const char* requisicao, std::ofstream& arquivoSaida) {
    ...
    // 1. Navega pela árvore encontrando o maior prefixo da requisição que existe.
    for (int i = 0; i < strlen(requisicao); ++i) {
        ...
        if (indice >= 0 && indice < TAMANHO_ALFABETO && noAtual->filhos[indice]) {
            ...
            // O caminho terminou, quebra o loop.
            break;
        }
    }

    // Se nenhum prefixo foi encontrado (ex: requisição começa com 'z' e não há palavras com 'z'),
    // ou se o prefixo encontrado não leva a nenhuma sugestão, apenas imprime a linha vazia.
    if (strlen(prefixoValido) == 0) {
        arquivoSaida << "\n";
        return;
    }
    
    // 2. Inicia a busca recursiva a partir do último nó válido encontrado.
    char bufferSugestao[TAMANHO_MAX_SUGESTAO];
    strcpy(bufferSugestao, prefixoValido);
    
    bool primeira = true;
    // O tamanho máximo é baseado na REQUISIÇÃO ORIGINAL.
    int tamanhoMax = strlen(requisicao) * 2;
    if (strlen(requisicao) == 0) {
        tamanhoMax = 0; // Sem limite prático se a requisição for vazia
    }
